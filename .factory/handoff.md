...
**Retry 2 final evidence (2 September 2026 UTC):** A fresh clone of runtime source `f458a63675e0af59692355273a44d1858ba3a7db` installed cleanly with `npm ci` and zero vulnerabilities. With the README's Tauri packages installed and a disk guard retaining at least 3.5 GiB free, every one of the 27 declared claim commands passed. `npm run lint`, `npm test` (7 Vitest and 24 Rust tests), `npm run build`, `npm run test:e2e` (44/44), and `npm audit --audit-level=high` also passed. The production build created both `dist/app` and `dist/site`.

**Current deployment:** Azure Static Web Apps deployment `0aadbbe3-3b9f-4180-834f-2da3a9044d06` deployed the clean-built `f458a63` site only to `sf-local-data-finder` in resource group `sociobot`. Cold `verify-url.sh` passed in 941 ms with no console/page errors (`.factory/qa-artifacts/retry2-refresh-live/`). Live Chromium plus Axe checks covered `/`, direct `/demo/?demo=1`, `/privacy/`, `/terms/`, and `/404.html` at 1366 px and 390 px: one h1 per route, no overflow, zero serious/critical violations. Header Demo navigation focused its h1 and announced the route; `/not-a-page` returns HTTP 404. The deployed footer still labels GitHub as external, contains no Azure provenance copy, and the install section explains SHA256 verification.

**Round-2 retry verification:** On the current disk-guard image, a fresh clone at `daa885d831843995adf111b73a5175f9d641bea8` completed `npm ci` with zero vulnerabilities, all 27 exact claim commands, `npm run lint`, `npm run check`, `npm test` (7 Vitest and 24 Rust), `cargo fmt --check`, strict `cargo clippy --all-targets -- -D warnings`, `npm run build`, `npm run test:e2e` (44/44), and `npm audit --audit-level=high`. After the README-documented Ubuntu Tauri packages were installed, the clean clone built its own Rust artifacts successfully. The local static build passed `/opt/fleet/lib/verify-url.sh` with zero console/page errors. Playwright's Axe integration is green; the separate Axe CLI is unavailable in this image because it cannot find a Chrome binary.

**Final deployment evidence:** Deployment `769a7a13-de92-4b98-902f-76deee328f25` published the rebuilt `7071141` static shell to the owned `sf-local-data-finder` Static Web App. Cold live `verify-url.sh` passed in 1,053 ms with no console/page errors; the captured page, desktop/mobile screenshots, and result JSON are in `.factory/qa-artifacts/round2-live-final/`. The live landing reports build `7071141`; it has the external-source label, the concrete SHA256 install explanation, and no public Azure provenance sentence. Live desktop and 390 px Chromium checks found one h1, no horizontal overflow, and zero serious/critical Axe findings on the landing, direct demo, legal routes, designed 404, and missing-route HTTP 404.
...
Fresh clone `/tmp/local-data-finder-polish2-mqdKZR/repo` at `8f520fd` completed `npm ci`, every one of the 27 exact `.factory/claims.json` commands, TypeScript/Cargo check, 7 Vitest and 24 Rust tests, production build, 44/44 Playwright desktop/mobile tests with Axe, privacy, demo, offline, focus, and touch coverage, and high-severity audit (zero vulnerabilities). The repair worktree also passed lint, rustfmt, and strict Clippy. The clean checkout shared the compact Cargo target only to keep the disposable filesystem below capacity; it still compiled and ran the clone's own desktop crate and test binary. Local static evidence is `.factory/qa-artifacts/polish-2-local/verify.json` (652 ms, zero console/page errors).
