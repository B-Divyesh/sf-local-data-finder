...
**Final deployment evidence:** Deployment `1a7374ff-1014-4503-ba22-db04987e2688` published the rebuilt `5f0b6b0` static shell to the owned `sf-local-data-finder` Static Web App. Cold live `verify-url.sh` passed in 825 ms with no console/page errors. The live landing reports build `5f0b6b0`; it has the external-source label, the concrete SHA256 install explanation, and no public Azure provenance sentence. Live desktop and 390 px Chromium checks found one h1, no horizontal overflow, and zero serious/critical Axe findings on the landing, direct demo, legal routes, designed 404, and missing-route HTTP 404.
