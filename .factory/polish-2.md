...
**Final deployment:** Azure Static Web Apps deployment `1a7374ff-1014-4503-ba22-db04987e2688` deployed the rebuilt `5f0b6b0` shell only to `sf-local-data-finder`. A cold `verify-url.sh` check of `https://local-data-finder.sociobot.in/` passed in 825 ms with no console or page errors. The live footer reports `build 5f0b6b0`, retains `Source on GitHub (external)`, and contains no `Azure AI Foundry` copy. Live Chromium checks at 1366 px and 390 px covered `/`, `/demo/?demo=1`, `/privacy/`, `/terms/`, `/404.html`, and `/not-a-page`; each had one h1, no overflow, and zero serious/critical Axe violations. The missing route returned HTTP 404.
